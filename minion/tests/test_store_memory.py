...
def _running_run(date: str = "2026-06-01", run_id: str = "RUN1") -> Run:
    return Run(run_id=run_id, date=date, status=RunStatus.running, started_at=_T0, steps=[])
...
def test_start_run_then_get_run_roundtrips() -> None:
    ...
    store.upsert_step(
        "2026-06-01",
        RunStep(name=StepName.gmail, status=RunStatus.success, started_at=_T0, ended_at=_T0),
    )
...
def test_steps_assembled_in_canonical_order() -> None:
    store = InMemoryRunStore()
    store.start_run(_running_run())
    for name in (StepName.scrape, StepName.gmail):  # inserted out of order
        store.upsert_step(
            "2026-06-01", RunStep(name=name, status=RunStatus.success, started_at=_T0)
        )
    got = store.get_run("2026-06-01")
    assert got is not None
    assert [s.name for s in got.steps] == [StepName.gmail, StepName.scrape]
...
def test_start_run_clears_prior_step_children() -> None:
    ...
    store.start_run(_running_run())
    store.upsert_step(
        "2026-06-01", RunStep(name=StepName.gmail, status=RunStatus.success, started_at=_T0)
    )
    store.start_run(_running_run(run_id="RUN2"))  # replay
    got = store.get_run("2026-06-01")
    assert got is not None
    assert got.run_id == "RUN2"
