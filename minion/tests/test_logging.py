...
def test_configure_logging_is_idempotent() -> None:
    configure_logging()
    configure_logging()
    # Count only the handler configure_logging installs: pytest's own capture handlers also sit
    # on this logger, and how many it attaches varies by pytest version.
    installed = [
        h
        for h in logging.getLogger(LOGGER_NAME).handlers
        if type(h).__name__ == "_StdoutJsonHandler"
    ]
