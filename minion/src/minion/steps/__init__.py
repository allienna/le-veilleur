...
`validate_output`), publishing (`imagen` / `github`), per-source analysis (`fiches`), and the
best-effort daily podcast (`podcast`), each wired to its injected client. Either way the
ordering is fixed by `STEP_ORDER`.
"""
