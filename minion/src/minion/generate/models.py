...
class ArticleFrontmatter(BaseModel):
    """Astro frontmatter for the generated article — the site's `articles` collection.

    Source of truth: site/src/content/config.ts. `image` is the hero filename the `imagen`
    step fills in and is empty at generation time. `sources` is absent by design: it is
    derived from the body's Sources list at serialization time rather than trusted to the
    model, so it cannot drift from what the article actually cites.
    """
