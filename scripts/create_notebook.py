#!/usr/bin/env python3
"""
le-veilleur — Create a NotebookLM notebook from a published article's sources.
...
Reads the day's published article, takes the URLs from its `## Sources` and
`## Pour aller plus loin` lists, deduplicates, filters tracking URLs, creates a
NotebookLM notebook named YYYY-MM-DD, adds each URL, and optionally generates a podcast.

Local tool, run on demand — it drives a browser-backed CLI (`nlm`) and has no place in the
cloud pipeline. It reads the published article rather than raw newsletters: the article is the
canonical record of what was actually cited, and there is no local scrape to read any more.
"""
...
ARTICLES_DIR = PROJECT_ROOT / "site" / "src" / "content" / "articles"

# A link entry in either list: `1. [Titre](URL)` or `- [Titre](URL) — pourquoi`.
_LINK_ENTRY = re.compile(r"^(?:\d+\.|-)\s*\[(?P<title>[^\]]+)\]\((?P<url>\S+?)\)")
...
def load_kept_sources(target_date: str) -> list[dict]:
    """Return [{title, url}] from the published article's Sources + Pour aller plus loin lists."""
    path = ARTICLES_DIR / f"{target_date}.md"
    if not path.exists():
        print(f"No published article at {path}", file=sys.stderr)
        sys.exit(1)

    sources: list[dict] = []
    inside = False
    for line in path.read_text(encoding="utf-8").splitlines():
        if line.startswith("## Sources") or line.startswith("## Pour aller plus loin"):
            inside = True
            continue
        if line.startswith("## "):  # any other section closes the collection
            inside = False
            continue
        if not inside:
            continue
        match = _LINK_ENTRY.match(line.strip())
        if match:
            sources.append({"title": match.group("title"), "url": match.group("url")})

    if not sources:
        print(f"No source links found in {path}", file=sys.stderr)
        sys.exit(1)
    return sources
