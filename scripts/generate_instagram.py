...
def parse_post(date_str: str) -> str:
    ...
    path = PROJECT_ROOT / "linkedin" / f"{date_str}.md"
    if not path.exists():
        return ""
...
def parse_article(date_str: str) -> dict | None:
    ...
    path = PROJECT_ROOT / "site" / "src" / "content" / "articles" / f"{date_str}.md"
    if not path.exists():
        return None
...
def extract_punches(date_str: str, article: dict) -> list[str]:
    ...
    raw_path = PROJECT_ROOT / "site" / "src" / "content" / "articles" / f"{date_str}.md"
    if raw_path.exists():
